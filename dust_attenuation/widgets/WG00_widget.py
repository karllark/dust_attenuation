...
class WG00_widget:
    def __init__(self):
        ...
        tau_V = wgt.Slider(rax, r"$\tau_V$", 0.5, 50, valinit=self.param["tau_V"])
    ...
    def update_sketch(self):
        ...
        self.rax_sketch.plot(X[mask], Y[mask], "*", color="orange", ms=10, zorder=1)
        ...
        if clumpy:
            ...
            x = r * np.cos(t)
            y = r * np.sin(t)
            self.rax_sketch.scatter(
                x, y, marker=marker, color="black", s=size, alpha=alpha, zorder=2
            )

        # Plot star ring
        star_ring = Wedge(
            0, Rs[1], 0, 360, width=Rs[1], alpha=alpha, color="lightyellow", zorder=0
        )
        ...
        if not clumpy:
            dust_ring = Wedge(
                0, Rd[1], 0, 360, width=1 - Rd[0], alpha=alpha, color="black", zorder=2
            )
        else:

            dust_ring = Wedge(
                0, Rd[1], 0, 360, width=Rd[1], alpha=alpha / 2, color="grey", zorder=2
            )
    ...
    def update_plot(self):
        if len(self.axatt.get_lines()) > 0:
            ...
            (self.plot_ext,) = self.axatt.plot(
                self.x, self.ext, ls="--", lw=3, color=color, label="Extinction"
            )
            (self.plot_att,) = self.axatt.plot(
                self.x,
                self.att / self.att_V,
                ls="-",
                lw=3,
            ...
            self.axatt.set_xlim(0.0, 10.5)
            self.axatt.set_ylim(0, 10)
            self.axatt.legend(prop={"size": 16})
            self.axatt.set_xlabel(r"1/$\lambda$ [$\mu m^{-1}$]", size=16)
            self.axatt.set_ylabel(r"A$_{\lambda}$ / A$_V$", size=16)
            self.axatt.tick_params(labelsize=14)
            self.axatt.set_title(
                "Attenuation curves from radiative transfer"
                + " model of\n  Witt & Gordon (2000)",
                size=20,
            )

            (self.plot_fesc,) = self.axFF.plot(
                self.x, self.fesc, ls="-", lw=1, color="black", label="esc"
            )

            (self.plot_fsca,) = self.axFF.plot(
                self.x, self.fsca, ls=":", lw=1, color="black", label="sca"
            )

            (self.plot_fdir,) = self.axFF.plot(
                self.x, self.fdir, ls="--", lw=1, color="black", label="dir"
            )

            self.axFF.set_xlim(0.0, 10.5)
            ...
            (self.plot_g,) = self.axalb.plot(
                self.x, self.g, ls="--", lw=1, color="black", label="g"
            )

            (self.plot_alb,) = self.axalb.plot(
                self.x, self.alb, ls="-", lw=1, color="black", label="albedo"
            )

            self.axalb.set_xlim(0.0, 10.5)
